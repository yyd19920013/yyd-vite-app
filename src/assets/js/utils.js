...
function ajaxWrap(config) {
  ...
  var errorPromise = {
    then: function () {
      console.error(
        '这是一个无效的then函数，如果要使用promise方式，不要在config对象里配置success、error、finally函数'
      );
      return this;
    },
    catch: function () {
      console.error(
        '这是一个无效的catch函数，如果要使用promise方式，不要在config对象里配置success、error、finally函数'
      );
      return this;
    },
    finally: function () {
      console.error(
        '这是一个无效的finally函数，如果要使用promise方式，不要在config对象里配置success、error、finally函数'
      );
      return this;
    },
  };
  ...
  config.timeout =
    config.timeout || config.timeout == 0 ? config.timeout : 20000;
...
function axiosWrap(config) {
  ...
  function createAxios(config) {
    ...
    var paramsOrData =
      method == 'get' || method == 'delete' || config.urlJointParams
        ? 'params'
        : 'data';
    var resultParams = !config.isForm
      ? config.params
      : jsonToStr(config.params);
    var configResult = {
      url: url,
      method: method,
  ...
  heartbeat: function () {
    ...
    if (!This.timer1) {
      ...
      This.timer1 = setTimeout(function () {
        This.timer1 = null;
        This.options.heartbeatJson &&
          This.ws &&
          This.ws.send(JSON.stringify(This.options.heartbeatJson));
...
function Scroll(obj, position, dis) {
  var position =
    ...
    position
      .toLowerCase()
      .replace(/^[a-z]{1}/, position.charAt(0).toUpperCase());
...
function openMoblieDebug(whiteList) {
  ...
  var open =
    hostname == 'localhost' ||
    hostname == '127.0.0.1' ||
    hostname == '172.16.21.92' ||
    ~whiteList.indexOf(hostname);
  var count = 0;
...
var idCardNo = {
  ...
  getIdCardNoInfo: function (idCardNo) {
    ...
    return this.checkIdCardNo(idCardNo)
      ? idCardNoInfo
      : this.getIdCardNoCheckInfo(idCardNo);
  },
  checkAddressCode: function (idCardNo) {
    ...
    return reg.test(addressCode) && this.citys[addressCode.substring(0, 2)]
      ? true
      : false;
  },
...
function secondFormat0(seconds, fmt, adjustFmt) {
  ...
  var dSeconds =
    dMinutes - iMinutes * aMinute ? dMinutes - iMinutes * aMinute : 0;
  ...
  for (var attr in time) {
    if (new RegExp('(' + attr + ')').test(fmt)) {
      ...
      value =
        result.length == 1
          ? value
          : zeroFill(value, result.length - value.length);
...
function amountFormat0(value, dLength, cLength) {
  ...
  if (Type(value) == 'number') {
    ...
    while (value[0].length > cLength) {
      arr.unshift(
        ',',
        value[0].splice(value[0].length - cLength, cLength).join('')
      );
    }
...
var cookie = {
  set: function (key, value, sec) {
    ...
    document.cookie =
      key + '=' + encodeURIComponent(value) + ';expires=' + oDate;
  },
  get: function (key) {
...
function strToJson(str) {
  var search =
    window && window.location && window.location.search.replace('?', '');
  var str = str || search || '';
  var arr = str.split('&');
  var json = {};
...
function alerts(str, msec, noMask) {
  ...
  oMask.style.cssText =
    'width:100%;height:100%;position:fixed;left:0;top:0;z-index:99999;';
  oWrap.style.cssText =
  ...
  for (var i = 0; i < arr.length; i++) {
    ...
    if (arr[i].reg) {
      ...
      if (
        condition &&
        regJson[arr[i].type] &&
        !regJson[arr[i].type].reg.test(arr[i].value)
      ) {
  ...
  endFn &&
    endFn({
      ...
      wrapTag: function (tagName, insert, objStyle, objProperty, objAttribute) {
        ...
        var oRange =
          selectedObj.rangeCount > 0 ? selectedObj.getRangeAt(0) : '';
      ...
      getNodeList: function (parent) {
        ...
        function getNodeListFn(parent) {
          ...
          for (var i = 0; i < childNodes.length; i++) {
            var isContains =
              selectedObj.containsNode(childNodes[i]) && childNodes[i].data;
      ...
      getCssText: function (parent) {
        ...
        function getCssTextFn(parent) {
          if (
            ...
            (!parent.style || !parent.style.cssText)
          )
            return;
  ...
  return {
    record: function () {
      ...
      setTimeout(function () {
        ...
        if (
          href != historyArr[historyArr.length - 1] ||
          length != historyLength[historyLength.length - 1]
        ) {
